...
month_data = month_data.combineByKey(lambda value: (value[0] if value[0] != None else 0, 1 if value[0] != None else 0,\
				...
				lambda x, value: (get_value(x[0]) + value[0], value[1] + 1, get_value(x[1])+value[2], 1 + value[3], get_value(x[3]) + value[4], 1 + value[5],\
					get_value(x[6])+value[6], 1 + value[7], get_value(x[8]) + value[8], get_value(x[9]) + value[9]),\
