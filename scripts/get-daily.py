...
month_data = month_data.combineByKey(lambda value: (value[0] if value[0] != None else 0, 1 if value[0] != None else 0,\
				...
				lambda x, value: (utils.get_value(x[0]) + value[0], value[1] + 1, utils.get_value(x[1])+value[2], 1 + value[3], utils.get_value(x[2]) + value[4], 1 + value[5],\
					utils.get_value(x[3])+value[6], 1 + value[7], utils.get_value(x[4]) + value[8], utils.get_value(x[5]) + value[9]),\
